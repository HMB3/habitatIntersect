...
## The package should import all the required packages
...
The aim is to create a pipeline that rapidly assesses the habitat suitability 
of large suites of species under current conditions. It could be used to
assess the distribution of any species (e.g. plants, reptiles, etc).
...
using a sample of ~40 insect species that are found in the forest of North Eastern NSW. 
...
The species data are downloaded as individual .Rdata files to the specified folders, which must exist first, without returning anything.
...
This pipeline combines occurrence points with raster data. It was developed using worldclim 
climate raster data, but it can take any set of rasters. The spatial data used to develop 
the workflow are on google drive - put this 
...
and recorded after 1950. We then trims the occurrence records to those inside the raster boundaries (i.e. species records in the ocean according to the Raster boundaries will be excluded).
...
# STEP 2a :: Create background points

\

To estiamte habitat suitability, we need to have as many species records as possible that
have the same spatial bias as the species we are analysing (REF). The simplest way to do this 
is to use species of the same type of organism we are analysing (in this case, insects.) 

Given there are only ~100 species in our anlaysis, we need more taxa to generate background points.
We're analysing insects, so we can get ALL the insect records from the ALA. A background point is 
simply then any geographic record (lat/lon) for a species other than the species we are 
analysing.
...
## Make a new function here that prcoesses the background points
ALA.BG = combine_background_records(background_df     = insect_background_records,
                                    species_list      = analysis_spp,
                                    record_type       = "ALA_BACKGROUND",
                                    keep_cols         = ALA_keep,
                                    world_raster      = worldclim_annual_temp)
