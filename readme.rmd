...
is to use species of the same type of organism we are analysing. 

Given there are only ~100 species in this anlaysis, we need more taxa to generate background points.
We're analysing insects, so we can get ALL the insect records from the ALA. A background point is 
simply then a geographic records (lat/lon) for any species we are NOT analysing.
