...
## Use a list 
analysis_spp <- str_trim(insect_data_families)
analysis_spp
...
download_ALA_all_species(species_list  = analysis_spp,
                         ...
                         download_path = "./data/ALA/Insects/",
...
To estimate habitat suitability, we need to have as many species records as possible that
have the same spatial bias as the taxa we are analysing (REF). The simplest way to do this 
is to use species of the same type of organism we are analysing (in this case, insects).

\

Given there are only ~100 species in our analysis, we need mnay more taxa to generate enough 
background points.We're analyzing insects, so we can get ALL the insect records from the ALA. 
A background point is simply then any geographic record (lat/lon) for a species, other than 
the species we are analysing.

\

```{r message=FALSE, echo=TRUE, warning=FALSE, eval=FALSE}

## Make a new function here that processes the background points
ALA.BG = combine_background_records(background_df  = insect_data_standard_columns_5th,
                                    species_list   = analysis_spp,
                                    record_type    = "ALA_BACKGROUND",
                                    keep_cols      = common_cols,
                                    world_raster   = worldclim_annual_temp,
                                    save_data      = TRUE,
                                    save_run       = "TEST_INSECTS",
                                    data_path      = "./data/ALA/Background_points/Insects/")
...
## Combine the  
BG.RASTER.CONVERT    = combine_records_extract(ala_df          = ALA.BG,
                                               site_df         = 'NONE',
                                               thin_records    = FALSE,
                                               template_raster = template_raster_1km_WGS84,
                                               world_raster    = worldclim_climate,
                                               prj             = CRS("+init=epsg:4326"),
                                               species_list    = unique(ALA.BG$searchTaxon),
...
## COMBO.RASTER.CONVERT = readRDS('./output/results/COMBO_RASTER_CONVERT_TEST_INSECTS.rds')
