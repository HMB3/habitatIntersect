...
## Insect species
target.insect.spp = target.insect.spp.df$searchTaxon %>%  unique() %>% str_trim() %>% .[!is.na(.)]


## Insect genera
target.insect.genera = target.insect.spp.df$Genus %>%  unique() %>% str_trim() %>% .[!is.na(.)]
...
## 3). OVERALL INVERT TAXA LISTS =============================================================


\

Now combine all the state PBI data into single lists :
