...
## Genera
all_insect_site_df_genera <- all_insect_site_df_species_sf %>%
  dplyr::select(-searchTaxon) %>% 
  dplyr::rename(searchTaxon = genus) %>% dplyr::select(searchTaxon, everything())


## Families
all_insect_site_df_families <- all_insect_site_df_species_sf %>%
  dplyr::select(-searchTaxon)  %>% 
  dplyr::rename(searchTaxon = family) %>% dplyr::select(searchTaxon, everything())
...
COMBO_RASTER_FAM_GEN_SPP_sf <- SpatialPointsDataFrame(coords      = COMBO.RASTER.FAM.GEN.SPP %>% 
                                                        dplyr::select(lon, lat) %>% as.matrix(),
                                                      data        = all_insect_site_df_species,
                                                      proj4string = CRS("+init=epsg:4326")) %>%
...
st_write(all_insect_site_df_species_sf, 
         dsn   = paste0(inv_records_dir, 'ALL_PBI_TAXA_POINTS.gpkg'), 
         quiet = TRUE)
