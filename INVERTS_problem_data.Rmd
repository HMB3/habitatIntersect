...
SDM.SPAT.OCC.BG.GDA <- saveRDS(ALA.LAND.INV.SPP, paste0(inv_results_dir, 'ALA_LAND_INV_SPP.rds'))
...
ALA.LAND.INV.FAM   <- ALA.LAND.INV.SPP %>% dplyr::select(-searchTaxon) %>% 
  dplyr::rename(searchTaxon = family)  %>% dplyr::s4elect(searchTaxon, everything())


## Check a few of the taxa now
Rayieria.basifer          <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Rayieria basifer', ]
Epimixia.vulturna         <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Epimixia vulturna', ]
Kirkaldyella.rugosa       <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Kirkaldyella rugosa', ]
Parmavitrina.planilabris  <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Parmavitrina planilabris', ]
Planorbacochlea.dandahra  <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Planorbacochlea dandahra', ]
Diorygopyx.simpliciclunis <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Diorygopyx simpliciclunis', ]
Diorygopyx.niger          <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Diorygopyx niger', ]
Asteron.grayi             <- ALA.LAND.INV.SPP %>% .[.$searchTaxon %in% 'Asteron.grayi', ]
