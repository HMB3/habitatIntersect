...
COMBO.SPP.GEN.FAM.ALA.PBI <- bind_rows(COMBO.RASTER.SPP.ALA.PBI, 
                                       COMBO.RASTER.GEN.ALA.PBI,
                                       COMBO.RASTER.FAM.ALA.PBI)

gc()


COMBO_RASTER_ALA_SPP_sf <- SpatialPointsDataFrame(coords      = COMBO.RASTER.SPP.ALA.PBI %>% 
                                                    dplyr::select(lon, lat) %>% as.matrix(),
                                                  data        = COMBO.RASTER.SPP.ALA.PBI,
                                                  proj4string = CRS("+init=epsg:4326")) %>%
...
COMBO_RASTER_ALA_SPP_sf <- COMBO_RASTER_ALA_SPP_sf %>% .[.$searchTaxon %in% analysis_taxa, ]


COMBO_RASTER_ALA_GEN_sf <- SpatialPointsDataFrame(coords      = COMBO.RASTER.GEN.ALA.PBI %>% 
                                                    dplyr::select(lon, lat) %>% as.matrix(),
                                                  data        = COMBO.RASTER.GEN.ALA.PBI,
                                                  proj4string = CRS("+init=epsg:4326")) %>%
...
COMBO_RASTER_ALA_FAM_sf <- SpatialPointsDataFrame(coords      = COMBO.RASTER.ALA.FAM %>% 
                                                    dplyr::select(lon, lat) %>% as.matrix(),
                                                  data        = COMBO.RASTER.ALA.FAM,
                                                  proj4string = CRS("+init=epsg:4326")) %>%
...
COMBO_RASTER_ALA_TAXA_sf <- COMBO_RASTER_ALA_TAXA_sf %>% .[.$searchTaxon %in% analysis_taxa, ]
...
st_write(COMBO_RASTER_ALA_SPP_sf,
         dsn    = paste0(inv_records_dir, 'TEST_ALA_SPP_POINTS.gpkg'),
         quiet  = TRUE,
         append = FALSE)


st_write(COMBO_RASTER_ALA_GEN_sf,
         dsn    = paste0(inv_records_dir, 'TEST_ALA_GEN_POINTS.gpkg'),
         quiet  = TRUE,
         append = FALSE)


st_write(COMBO_RASTER_ALA_FAM_sf,
         dsn    = paste0(inv_records_dir, 'TEST_ALA_FAM_POINTS.gpkg'),
         quiet  = TRUE,
         append = FALSE)
...
st_write(COORD_CLEAN_sf, 
         dsn    = paste0(inv_records_dir, 'TEST_CLEAN_TAXA_POINTS.gpkg'), 
         quiet  = TRUE, 
         append = FALSE)
...
st_write(SDM.SPAT.OCC.BG.TARG.INV, 
