...
ALA.LAND.INV.SPP <- format_ala_dump(ALA_table     = head(Insects_ALA, 10000),
                                    record_type   = "ALA",
                                    keep_cols     = ALA_keep,
                                    year_filt     = FALSE,
                                    unique_cells  = FALSE,
...
COMBO.RASTER.ALA.SPP = combine_records_extract(ala_df           = ALA_LAND_INV_SPP_sf,
                                               add_sites        = TRUE,
                                               filter_taxo      = FALSE,
...
COMBO.RASTER.ALA.SPP.GENUS <- COMBO.RASTER.ALA.SPP %>% dplyr::select(-searchTaxon) %>% 
  dplyr::rename(searchTaxon = genus)   %>% dplyr::select(searchTaxon, everything())


COMBO.RASTER..ALA.SPP.FAM   <- COMBO.RASTER.ALA.SPP %>% dplyr::select(-searchTaxon) %>% 
  dplyr::rename(searchTaxon = family)  %>% dplyr::select(searchTaxon, everything())


## Now bind the site data to each
COMBO.RASTER.SPP.ALA.PBI <- bind_rows(COMBO.RASTER.ALA.SPP, COMBO.RASTER.PBI.SPP)
COMBO.RASTER.SPP.ALA.GEN <- bind_rows(COMBO.RASTER.ALA.GEN, COMBO.RASTER.PBI.SPP)
