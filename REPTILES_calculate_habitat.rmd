...
sdm_threshold_features <- list.files(path    = './output/reptile_maxent/Habitat_suitability/SDM_thresholds/',
...
for(feat in sdm_threshold_features) { 
  
  layer_name <- sdm_threshold_list[[feat]]
  tempData   <- st_read(dsn   = paste0(threshold_folders, 'reptiles_sdm_thresholds_combo.gpkg'), 
                        layer = layer_name) 
  
  message('assigning ', feat, ' layer')
  assign (layer_name, tempData, envir = .GlobalEnv) 
...
FESM_east_20m   <- st_read('./data/Remote_sensing/FESM/Fire_perimeters_for_forests_and_woodlands.shp') %>% 
  st_transform(., st_crs(3577)) %>% filter(!st_is_empty(.)) %>% as_Spatial()


## NIAFED data is much coarser and has more empty geometries
Burnt_unburnt   <- 
  
  st_read('./data/Remote_sensing/NIAFED/National_Indicative_Aggregated_Fire_Extent_Dataset_v20200211.shp') %>%
...
AUS_forest_RS_feat          = st_read('./data/Remote_sensing/Veg_data/Forest_cover/Aus_forest_cover_east_coast.shp') %>% 
  st_transform(., st_crs(3577)) %>% filter(!st_is_empty(.)) %>% as_Spatial()
