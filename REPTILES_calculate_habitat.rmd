...
sdm_dir       <- './output/reptile_maxent/Habitat_suitability/SDM_point_data/'
...
target_reptiles  <- c('Phyllurus platurus',  'Eulamprus tympanum',      'Hoplocephalus bungaroides', 'Drysdalia rhodogaster', 
                      ...
                      'Morelia spilota',     'Pseudechis porphyriacus', 'Varanus rosenbergi',        'Eulamprus tympanum')
...
# 3). Estimate the habitat burnt by the 2019-2020 Fires per taxon
...
calculate_taxa_habitat_fire_features(taxa_list          = sort(REPTILES.MAXENT.RESULTS$searchTaxon),
...
REPTILE.FIRE.list   <- list.files('./output/reptile_maxent/Habitat_suitability/FESM_SDM_intersect/', 
                                  pattern     = '_SDM_intersect_Fire.csv', 
                                  full.names  = TRUE, 
                                  recursive   = TRUE)


REPTILE.FIRE.VEG.list <- list.files('./output/reptile_maxent/Habitat_suitability/FESM_SDM_intersect/', 
                                    pattern     = '_SDM_VEG_intersect_Fire.csv', 
                                    full.names  = TRUE, 
                                    recursive   = TRUE) 


## Now combine the SUA tables for each species into one table 
REPTILE.FIRE.TABLE <- REPTILE.FIRE.list %>%
...
REPTILE.FIRE.VEG.TABLE <- REPTILE.FIRE.VEG.list %>%
...
REPTILE.FIRE.TABLE <-  REPTILE.FIRE.TABLE[REPTILE.FIRE.TABLE$Taxa %in% 
                                            sort(REPTILES.MAXENT.RESULTS$searchTaxon) , ] %>%  
  .[complete.cases(.), ]


REPTILE.FIRE.VEG.TABLE <-  REPTILE.FIRE.VEG.TABLE[REPTILE.FIRE.VEG.TABLE$Taxa %in% 
                                            sort(REPTILES.MAXENT.RESULTS$searchTaxon) , ] %>%  
  .[complete.cases(.), ]
...
write_csv(REPTILE.FIRE.TABLE, 
          './output/reptile_maxent/Habitat_suitability/FESM_SDM_intersect/REPTILE_TAXA_SDM_intersect_Fire.csv')


write_csv(REPTILE.FIRE.VEG.TABLE, 
