...
SDM.REPTILE.SPAT.OCC.BG.GDA = readRDS('./output/results/SDM_SPAT_OCC_BG_GDA_ALL_TARGET_REPTILE_TAXA.rds')
...
## Run species-level models for plants
run_sdm_analysis(taxa_list               = target.host.plants,
                 taxa_level              = 'species',
                 maxent_dir              = 'output/veg_climate_topo_maxent/full_models',     
                 bs_dir                  = 'output/veg_climate_topo_maxent/back_sel_models',
                 sdm_df                  = SDM.SPAT.OCC.BG.GDA,
...
tryCatch(
  ...
  error = function(cond) {
    ...
    file.create(file.path("output/veg_climate_topo_maxent/back_sel_models/mapping_failed_current.txt"))
    cat(cond$message, file = file.path("output/veg_climate_topo_maxent/back_sel_models/inv_mapping_failed_current.txt"))
...
host_plants <- read_excel('./output/veg_climate_topo_maxent/Habitat_suitability/NENSW_INVERTEBRATES_SPATIAL_DATA_LUT_SEP2021.xlsm',
                          sheet = 'NENSW_INV_TAXA_ASSOCIATIONS') %>% filter(Target_taxon == "Yes") %>% 
  select(searchTaxon, Host_Plant_taxon) 
