...
# analysis_taxa <- str_trim(target.insect.spp[1:10])
analysis_taxa <- str_trim(all.insect.families) %>% unique() 
analysis_taxa
...
## And a stack of grids for vegetation for the study area in NE_NSW, in GDA Albers projection
study.veg.current <- stack(
...
names(study.climate.current) <- names(worldclim_climate)
names(aus.veg.grids)         <- names(study.veg.current) <- c("Plant_cover_fraction_0_5m", 
...
study.climate.veg.grids <- stack(study.climate.current, study.veg.current)
