...
##
download_ALA_all_species(taxa_list      = analysis_taxa,
...
download_ALA_all_genera(taxa_list    = analysis_taxa,
                        your_email      = 'hugh.burley@gmail.com',
...
ALA.LAND.FAM <- combine_ala_records(taxa_list         =  sort(all.insect.families)[45],
...
COMBO.RASTER.FAM = combine_records_extract(ala_df           = ALA.LAND.FAM,
                                           site_df          = all_insect_site_df_families %>% filter(searchTaxon == "Miridae"),
                                           thin_records     = FALSE,
                                           template_raster  = template_raster_1km_WGS84,
                                           world_raster     = aus.climate.veg.grids,
                                           prj              = CRS("+init=epsg:4326"),
                                           taxa_list        = sort(all.insect.families)[45],
                                           taxa_level       = 'family',   
...
COMBO.RASTER.GEN = combine_records_extract(ala_df           = ALA.LAND.GEN,
                                           site_df          = all_insect_site_df_genera %>% filter(searchTaxon == "Malandiola"),
                                           thin_records     = FALSE,
                                           template_raster  = template_raster_1km_WGS84,
                                           world_raster     = aus.climate.veg.grids,
                                           prj              = CRS("+init=epsg:4326"),
                                           taxa_list        = sort(target.insect.genera)[38],
                                           taxa_level       = 'genus', 
...
Lygaeidae_ALA           <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Lygaeidae'  | family == 'Lygaeidae')
Malandiola_ALA          <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Malandiola' | genus  == 'Malandiola')
Miridae_ALA             <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Miridae'    | family == 'Miridae')
Nysius_vinitor_ALA      <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Nysius vinitor'  | species == 'Nysius vinitor')
Onthophagus_turrbal_ALA <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Onthophagus turrbal'  | family == 'Onthophagus turrbal')
Oxycarenidae_ALA        <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Oxycarenidae'  | family == 'Oxycarenidae')
Oxycarenus_ALA          <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Oxycarenus'    | genus == 'Oxycarenus')

Pachygronthidae_ALA     <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Pachygronthidae'  | family == 'Pachygronthidae')
Pentatomidae_ALA        <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Pentatomidae'     | family == 'Pentatomidae')
Scutelleridae_ALA       <- SDM.SPAT.OCC.BG.GDA %>% as.data.frame () %>% filter(searchTaxon == 'Scutelleridae'    | family == 'Scutelleridae')
...
SDM.ALL.PLA.SPP.BG = readRDS('./output/results/SDM_SPAT_OCC_BG_HOST_PLANT_SPP.rds') %>% as.data.frame() %>%
...
saveRDS(SDM.SPAT.OCC.BG.GDA, './output/results/SDM_SPAT_OCC_BG_GDA_ALL_TAXA.rds')

writeOGR(obj    = SDM.SPAT.OCC.BG.GDA,
         dsn    = 'G:/North_east_NSW_fire_recovery/output/results',
         layer  = 'SDM_SPAT_OCC_BG_GDA_ALL_INSECT_TAXA', 
