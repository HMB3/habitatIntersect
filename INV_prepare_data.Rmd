...
analysis_test <- c('Lygaeidae',           'Malandiola',       'Miridae',      'Nysius vinitor', 'Onthophagus turrbal', 'Oxycarenidae', 
                   ...
                   'Mutusca brevicornis', 'Rayieria basifer', 'Wallabicoris', 'Molycria',       'Figulus')
...
## Combine Family ALA data ~ 50% of records are knocked out by cleaning  
ALA.LAND.FAM <- combine_ala_records(taxa_list         =  analysis_test,
...
ALA.LAND.GEN <- combine_ala_records(taxa_list         = analysis_test,
...
ALA.LAND.SPP <- combine_ala_records(taxa_list         = analysis_test,
...
SDM.SPAT.OCC.BG <- prepare_sdm_table(coord_df          = COORD.CLEAN,
                                     taxa_list         = analysis_test,
                                     
                                     sdm_table_vars    = c('searchTaxon', 'species',  'genus', 'family', 'lon', 'lat', 'SOURCE', 'SPAT_OUT',
                                                           names(aus.climate.veg.grids)),
                                     save_run          = "TARGET_HOST_PLANTS",
...
saveRDS(SDM.SPAT.OCC.BG.GDA, './output/results/SDM_SPAT_OCC_BG_GDA_TARGET_HOST_PLANTS.rds')


writeOGR(obj    = SDM.SPAT.OCC.BG.GDA,
         dsn    = 'G:/North_east_NSW_fire_recovery/output/results',
         layer  = 'SDM_SPAT_OCC_BG_GDA_TARGET_HOST_PLANTS', 
...
## Where are the records going missing?
## Are the still there in coord.clean?
## Isolate the records


## Check the records for a subset of taxa in the final data (ALA + SITE)
Lygaeidae_ALA            <- SDM.SPAT.OCC.BG   %>% as.data.frame () %>% filter(searchTaxon == 'Lygaeidae'  | family == 'Lygaeidae')
Malandiola_ALA           <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Malandiola' | genus  == 'Malandiola')
Miridae_ALA              <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Miridae'    | family == 'Miridae')
Nysius_vinitor_ALA       <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Nysius vinitor'  | species == 'Nysius vinitor')
Onthophagus_turrbal_ALA  <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Onthophagus turrbal' | family == 'Onthophagus turrbal')
Oxycarenidae_ALA         <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Oxycarenidae'  | family == 'Oxycarenidae')
Oxycarenus_ALA           <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Oxycarenus'    | genus == 'Oxycarenus')

Pachygronthidae_ALA      <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Pachygronthidae'  | family == 'Pachygronthidae')
Pentatomidae_ALA         <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Pentatomidae'     | family == 'Pentatomidae')
Scutelleridae_ALA        <- SDM.SPAT.OCC.BG  %>% as.data.frame () %>% filter(searchTaxon == 'Scutelleridae'    | family == 'Scutelleridae')


## Check the records for a subset of taxa in the SITE data
Lygaeidae_site           <- all_insect_site_df_families %>% filter(searchTaxon == 'Lygaeidae')
Malandiola_site          <- all_insect_site_df_genera   %>% filter(searchTaxon == 'Malandiola')
Miridae_site             <- all_insect_site_df_families %>% filter(searchTaxon == 'Miridae')
Nysius_vinitor_site      <- all_insect_site_df_species  %>% filter(searchTaxon == 'Nysius vinitor')
Onthophagus_turrbal_site <- all_insect_site_df_species  %>% filter(searchTaxon == 'Onthophagus turrbal')


Oxycarenidae_site        <- all_insect_site_df_families %>% filter(searchTaxon == 'Oxycarenidae')
Pachygronthidae_site     <- all_insect_site_df_families %>% filter(searchTaxon == 'Pachygronthidae')
Pentatomidae_site        <- all_insect_site_df_families %>% filter(searchTaxon == 'Pentatomidae')
Scutelleridae_site       <- all_insect_site_df_families %>% filter(searchTaxon == 'Scutelleridae')


## Check a few of them
table(Lygaeidae_ALA$SOURCE)
nrow(Lygaeidae_site)

table(Malandiola_ALA$SOURCE)
nrow(Malandiola_site)

table(Miridae_ALA$SOURCE)
nrow(Miridae_site)

table(Nysius_vinitor_ALA$SOURCE)
nrow(Nysius_vinitor_site)

table(Onthophagus_turrbal_ALA$SOURCE)
nrow(Onthophagus_turrbal_site)

table(Oxycarenidae_ALA$SOURCE)
nrow(Oxycarenidae_site)

table(Pachygronthidae_ALA$SOURCE)
nrow(Pachygronthidae_site)

table(Scutelleridae_ALA$SOURCE)
nrow(Scutelleridae_site)
