...
# 1km Climate, Soil and terrain variables
...
# This takes ages in R.....send this to Adrian?
...
study_precip_250m <- raster::stack(
  list.files('./data/Bushfire_indices/R_outputs/250m/STUDY_AREA/Precip', pattern =".tif", full.names = TRUE))
...
study_temp_250m <- raster::stack(
  list.files('./data/Bushfire_indices/R_outputs/250m/STUDY_AREA/Temp',   pattern =".tif", full.names = TRUE))
...
study_soil_250m <- raster::stack(
  list.files('./data/Bushfire_indices/R_outputs/250m/STUDY_AREA/Soil',   pattern =".tif", full.names = TRUE))
...
study_geology_250m <- raster::stack(
  list.files('./data/Bushfire_indices/R_outputs/250m/STUDY_AREA/Geology', pattern =".tif", full.names = TRUE))
...
study_topo_250m <- raster::stack(
  list.files('./data/Bushfire_indices/R_outputs/250m/STUDY_AREA/Topo',    pattern =".tif", full.names = TRUE))
...
study_terrain_250m <- raster::stack(
  list.files('./data/Bushfire_indices/R_outputs/250m/STUDY_AREA/Indices', pattern =".tif", full.names = TRUE))
...
study.grids.current.250m <- stack(study_precip_250m,
                                  study_temp_250m#,
                                  #aus_soil_250m,
                                  #aus_geology_250m
)
...
study.veg.grids.250m <- stack(
  
  list.files('./data/Remote_sensing/Veg_data/height_and_cover/Study/250m', 
             '_study.tif', full.names = TRUE))


names(aus.veg.grids.250m) <- names(study.veg.grids.250m) <- c("Plant_cover_fraction_0_5m", 
                                                              "Plant_cover_fraction_5_10m",  
                                                              "Plant_cover_fraction_10_30m",      
                                                              "Plant_cover_fraction_30m",
                                                              "Total_Plant_cover_fraction",  
...
study.climate.veg.grids.250m <- stack(study.grids.current.250m, study.veg.grids.250m)
aus_annual_precip            <- raster('./data/Bushfire_indices/R_outputs/250m/AUS/Extra/Annual_precip_WGS84.tif')


# We also need a template raster of 250m * 250m cells, used to filter 
# records to 1 per each 250m cell. This raster needs to have the same extent (global) 
# resolution (1km) and projection (WGS84) of the data used to analyse the taxa 
# distributions. It should have a value of 1 for land, and NA for the ocean. 
# This takes ages in R.....
template_raster_250m_mol     <- aus_annual_precip


## Should be 1km*1km, It should have a value of 1 for land, and NA for the ocean
template_raster_250m_mol[template_raster_250m_mol > 0] <- 1
template_raster_250m_mol[template_raster_250m_mol < 0] <- 1
xres(template_raster_250m_mol);projection(template_raster_250m_mol)
...
Hoplocephalus_bungaroides_bionet <- read_csv('./data/ALA/Reptiles/Hoplocephalus bungaroides_Bionet.csv') %>% 
  
  dplyr::rename(searchTaxon = ScientificName,
                lat         = Latitude_GDA94,
                lon         = Longitude_GDA94) %>% dplyr::select(searchTaxon, lat, lon) %>% 
...
COMBO.RASTER.SPP <- combine_records_extract(ala_df           = ALA.LAND.BACKG.SPP,
                                            ...
                                            thin_records     = FALSE,
                                            template_raster  = template_raster_250m_mol,
...
SPATIAL.CLEAN = check_spatial_outliers(occ_df       = head(COORD.CLEAN, 500),
                                       land_shp     = LAND,
                                       site_records = TRUE, 
                                       site_flag    = 'BIONET',  
...
SDM.SPAT.OCC.BG.REPTILE.MOL <- readRDS('./output/results/SDM_SPAT_OCC_BG_GDA_ALL_TARGET_REPTILE_TAXA.rds') %>% as.data.frame() %>%
...
SDM.SPAT.OCC.ALL.REPTILE.BG <- prepare_sdm_table(coord_df          = COORD.CLEAN,
                                                 ...
                                                 background_points = SDM.SPAT.OCC.BG.REPTILE.MOL, 
                                                 save_data         = TRUE,
...
## And save the big table data to be a projected coordinate system, not mollweide for use in Australia
SDM.SPAT.OCC.TARGET.REPTILE.BG   <- readRDS('./output/reptile_maxent/results/SDM_SPAT_OCC_BG_ALL_REPTILE_TAXA.rds')
SDM.SPAT.OCC.BG.REPTILE.GDA      <- spTransform(SDM.SPAT.OCC.TARGET.REPTILE.BG,  CRS(sp_epsg3577))
crs(SDM.SPAT.OCC.BG.REPTILE.GDA) <- sp_epsg3577
saveRDS(SDM.SPAT.OCC.BG.REPTILE.GDA, './output/reptile_maxent/results/SDM_SPAT_OCC_BG_REPTILE_GDA.rds')
