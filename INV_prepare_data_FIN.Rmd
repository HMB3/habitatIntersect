...
## These packages are not on cran, but are needed - Fin you'll need to load them
library(devtools)
...
## Create documentation and install package - run this too, but only once
...
## Set temporary raster dir for the terra package - need to create these folders
...
host_back_dir <- 'output/plant_maxent/back_sel_models/'
...
## Topography
aus.vbf   <- raster('./data/Topography/mrvbf_int_1km.tif')
study.vbf <- raster('./data/Topography/mrvbf_int_study_1km.tif')
...
ALA.LAND.FAM <- combine_ala_records(taxa_list         =  target.insect.families[1:2],
...
ALA.LAND.GEN <- combine_ala_records(taxa_list         = target.insect.genera[1:2],
...
ALA.LAND.SPP <- combine_ala_records(taxa_list         = target.insect.spp[1:2],
...
## Try combing the records into a "super table" with all taxonomic levels...
...
# saveRDS(GLOB.NICHE.ALL,   './output/plant_maxent/Habitat_suitability/GLOBAL_NICHES_TARG_PLANT_TAXA.rds')
# write_csv(GLOB.NICHE.ALL, './output/plant_maxent/Habitat_suitability/GLOBAL_NICHES_TARG_PLANT_TAXA.csv')
...
# SDM.SPAT.OCC.BG          <- readRDS('./output/results/SDM_SPAT_OCC_BG_GDA_ALL_TARGET_INVERT_TAXA.rds')
# SDM.SPAT.OCC.BG.GDA      <- spTransform(SDM.SPAT.OCC.BG,  CRS(sp_epsg3577))
# crs(SDM.SPAT.OCC.BG.GDA) <- sp_epsg3577
# saveRDS(SDM.SPAT.OCC.BG.GDA, './output/results/SDM_SPAT_OCC_BG_GDA_ALL_TARGET_INVERT_TAXA.rds')
# saveRDS(SDM.SPAT.OCC.BG.GDA, './output/results/SDM_SPAT_OCC_BG_GDA_ALL_TARGET_PLANT_TAXA.rds')


## Save the big table of all taxa, including background taxa, as a SPDF
writeOGR(obj    = SDM.SPAT.OCC.BG.GDA,
         dsn    = 'G:/GITHUB/North_east_NSW_fire_recovery/output/results',
...
SDM.SPAT.OCC.BG.TARG.GDA <- SDM.SPAT.OCC.BG.GDA %>% .[.$searchTaxon %in% MAXENT.RESULTS$searchTaxon,]
...
writeOGR(obj    = SDM.SPAT.OCC.BG.TARG.GDA,
         dsn    = 'G:/GITHUB/North_east_NSW_fire_recovery/output/results',
         layer  = 'SDM_SPAT_OCC_BG_GDA_ONLY_ANALYSIS_INVERT_TAXA', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE)

writeOGR(obj    = SDM.SPAT.OCC.BG.TARG.FAM,
         dsn    = 'G:/GITHUB/North_east_NSW_fire_recovery/output/results',
         layer  = 'SDM_SPAT_OCC_BG_GDA_ONLY_ANALYSIS_INVERT_FAMILIES', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE)

writeOGR(obj    = SDM.SPAT.OCC.BG.TARG.GEN,
         dsn    = 'G:/GITHUB/North_east_NSW_fire_recovery/output/results',
         layer  = 'SDM_SPAT_OCC_BG_GDA_ONLY_ANALYSIS_INVERT_GENERA', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE)

writeOGR(obj    = SDM.SPAT.OCC.BG.TARG.SPP,
         dsn    = 'G:/GITHUB/North_east_NSW_fire_recovery/output/results',
         layer  = 'SDM_SPAT_OCC_BG_GDA_ONLY_ANALYSIS_INVERT_SPECIES', 
         driver = 'ESRI Shapefile', overwrite_layer = TRUE)
...
for(taxa in target.insect.families) {
  ...
  if(taxa %in% unique(SDM.SPAT.OCC.BG.GDA$searchTaxon)) {
    ...
    taxa_shp <- paste0('G:/GITHUB/North_east_NSW_fire_recovery/output/veg_climate_topo_maxent/Habitat_suitability/SDM_point_data/',
                       taxa, '_SDM_points.shp')
    
    if(!file.exists(taxa_shp)) {
      
      message('Subsetting ', taxa, ' shapefile')
      taxa_occ <- subset(SDM.SPAT.OCC.BG.GDA, searchTaxon == taxa | family == taxa) %>% 
        spTransform(., crs(sp_epsg3577))
      
      writeOGR(obj    = taxa_occ,
               dsn    = 'G:/GITHUB/North_east_NSW_fire_recovery/output/veg_climate_topo_maxent/Habitat_suitability/SDM_point_data',
...
for(taxa in target.insect.genera) {
  ...
  if(taxa %in% unique(SDM.SPAT.OCC.BG.GDA$searchTaxon)) {
    ...
    if(!file.exists(taxa_shp)) {
      ...
      message('Subsetting ', taxa, ' shapefile')
      taxa_occ <- subset(SDM.SPAT.OCC.BG.GDA, searchTaxon == taxa | genus == taxa) %>% 
        spTransform(., crs(sp_epsg3577))
...
for(taxa in target.insect.spp) {
  ...
  if(taxa %in% unique(SDM.SPAT.OCC.BG.GDA$searchTaxon)) {
    ...
    if(!file.exists(taxa_shp)) {
      ...
      message('Subsetting ', taxa, ' shapefile')
      taxa_occ <- subset(SDM.SPAT.OCC.BG.GDA, searchTaxon == taxa | species == taxa) %>% 
        spTransform(., crs(sp_epsg3577))
