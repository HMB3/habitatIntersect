...
#' 
#' @title Threshold current maxent predictions
  ...
  species_list %>%
    ...
    lapply(function(taxa) {
      ...
      if(file.exists(current_file)) {
          ...
          ## taxa = taxa_list[3]
          message("Creating threshold rasters for ", taxa)
          ...
          current_thresh =  sprintf('%s/%s/full/%s_%s%s.tif', maxent_path,
                                    taxa, taxa, "current_suit_not_novel_above_", thresh)
